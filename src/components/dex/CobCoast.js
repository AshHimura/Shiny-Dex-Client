...
import { getCobaltPokemon } from "../management/DexManager"
...
export const CobaltCoastlands = () => {
    const loggedUser = parseInt(localStorage.getItem("user_id"))
    //CobaltPoke state holds array of all pokemon in the region from useEffect fetch
    const [CobaltPoke, setCobaltPoke] = useState([])
    ...
    useEffect(
        () => {
            getCobaltPokemon().then(data => setCobaltPoke(data))
    ...
    const handleUserSelect = (evt) => {
        const view = CobaltPoke.find(info => {
        ...
        <>
            ...
            loggedUser !== caughtObj?.user?.user ?
                (openModal ? <Modal closeModal={setOpenModal} createCatch={createCatch} compareAndStop={compareAndStop} 
                    ...
                    <DexData confirmSelectedPoke={confirmSelectedPoke} caughtObj={caughtObj} />  </>}
            <form className="cobo">
                <h3 className="cobo-pokename">
                    <select value={confirmSelectedPoke.id} id={CobaltPoke.id} className="cobo-names" onChange={(e) => { handleFindCatch(e); handleUserSelect(e); compareAndStop(e); }}>
                        <option value="" >Select a Pokemon</option>
                        {
                            CobaltPoke.map(cobalt => {
                                return <option className="coboPokemon" key={cobalt.id} value={cobalt.id}>
                                    {cobalt.name}
                                </option>
                            })
                        }
