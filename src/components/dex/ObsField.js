...
import { getObsidianPokemon } from "../management/DexManager"
...
export const ObsidianFieldlands = () => {
    const loggedUser = parseInt(localStorage.getItem("user_id"))
    //ObsidianPoke state holds array of all pokemon in the region from useEffect fetch
    const [ObsidianPoke, setObsidianPoke] = useState([])
    ...
    useEffect(
        () => {
            getObsidianPokemon().then(data => setObsidianPoke(data))
    ...
    const handleUserSelect = (evt) => {
        const view = ObsidianPoke.find(info => {
        ...
        <>
            ...
            loggedUser !== caughtObj?.user?.user ?
                (openModal ? <Modal closeModal={setOpenModal} createCatch={createCatch} compareAndStop={compareAndStop} 
                    ...
                    <DexData confirmSelectedPoke={confirmSelectedPoke} caughtObj={caughtObj} />  </>}
            <form className="obs">
                <h3 className="obs-pokename">
                    <select value={confirmSelectedPoke.id} id={ObsidianPoke.id} className="obs-names" onChange={(e) => { handleFindCatch(e); handleUserSelect(e); compareAndStop(e); }}>
                        <option value="" >Select a Pokemon</option>
                        {
                            ObsidianPoke.map(obsidian => {
                                return <option className="obsPokemon" key={obsidian.id} value={obsidian.id}>
                                    {obsidian.name}
                                </option>
                            })
                        }
